...
const LESSON_TYPES: Record<string, { label: string; color: string; bg: string; border: string }> = {
  REGULAR: { label: 'Lecture / Class', color: 'text-blue-700', bg: 'bg-blue-50', border: 'border-blue-200' },
  PRACTICAL: { label: 'Practical (TP)', color: 'text-emerald-700', bg: 'bg-emerald-50', border: 'border-emerald-200' },
  LAB: { label: 'Lab Session', color: 'text-teal-700', bg: 'bg-teal-50', border: 'border-teal-200' },
  TUTORIAL: { label: 'Tutorial (TD)', color: 'text-indigo-700', bg: 'bg-indigo-50', border: 'border-indigo-200' },
  BREAK: { label: 'Morning Recess', color: 'text-amber-700', bg: 'bg-amber-50', border: 'border-amber-200' },
  LUNCH: { label: 'Lunch Break', color: 'text-orange-700', bg: 'bg-orange-50', border: 'border-orange-200' },
  ASSEMBLY: { label: 'School Assembly', color: 'text-purple-700', bg: 'bg-purple-50', border: 'border-purple-200' },
  EXAM: { label: 'Assessment / Exam', color: 'text-rose-700', bg: 'bg-rose-50', border: 'border-rose-200' },
  STUDY: { label: 'Study Hall', color: 'text-sky-700', bg: 'bg-sky-50', border: 'border-sky-200' },
  SPORTS: { label: 'Physical Ed. / Sports', color: 'text-lime-700', bg: 'bg-lime-50', border: 'border-lime-200' },
  OTHER: { label: 'Activity', color: 'text-slate-700', bg: 'bg-slate-50', border: 'border-slate-200' },
};
  ...
  useEffect(() => {
    const fetchTimetable = async () => {
      try {
        ...
        setTeacherName(userRes.data?.name || userRes.data?.username || 'Instructor');
      } catch (error) {
  ...
  const handleExportPDF = () => {
    ...
    try {
      const timetableData: any = {
        className: 'Teacher Timetable: ' + teacherName,
      ...
      toast.success('Teaching schedule exported to PDF');
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#F8FAFC] p-6 lg:p-10 space-y-8">
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="p-5 rounded-3xl border border-slate-100 shadow-sm bg-white">
          ...
          <p className="text-2xl font-black text-blue-600 mt-2">{weeklyHours} hours / week</p>
        </Card>
        ...
        <Card className="p-5 rounded-3xl border border-slate-100 shadow-sm bg-white">
          ...
          <p className="text-2xl font-black text-purple-600 mt-2">{totalSessions} classes</p>
        </Card>
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            {currentSession && (
              <Badge className="bg-white/20 text-white border-none font-bold text-[10px]">
                {currentSession.remainingMinutes} min remaining
              </Badge>
            )}
          </div>
          ...
          {currentSession ? (
            ...
            <div className="py-2">
              <h3 className="text-base font-black text-slate-700">
                No teaching session in progress
              </h3>
              <p className="text-xs text-slate-400 font-bold mt-1">
                Prepare your next lesson materials.
              </p>
            </div>
          )}
        </div>
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              {nextSession && (
                <Badge className="bg-slate-100 text-slate-700 font-bold text-[10px] border border-slate-200">
                  In {nextSession.minutesUntil} min
                </Badge>
              )}
            </div>
            ...
            {nextSession ? (
              ...
              <div className="py-2">
                <h3 className="text-base font-black text-slate-600">
                  All teaching sessions completed for today
                </h3>
                <p className="text-xs text-slate-400 font-bold mt-1">
                  No remaining classes scheduled today.
                </p>
              </div>
            )}
          </div>
            ...
            {dailySchedule.length === 0 && (
              <div className="h-64 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-[2.5rem] text-slate-400 bg-white">
                ...
                <p className="font-black uppercase text-xs tracking-widest text-slate-400">
                  No teaching scheduled for {DAY_LABELS[activeDay]}
                </p>
              </div>
            )}
          </div>
