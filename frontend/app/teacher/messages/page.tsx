...
export default function TeacherMessagesPage() {
  ...
  // Fetch recipients (admins and parents)
  useEffect(() => {
    const fetchRecipients = async () => {
      try {
        const res = await api.get('/users?pagination[limit]=100');
        const list = Array.isArray(res.data) ? res.data : (res.data?.results || []);
        // Filter out self and only keep ADMIN and PARENT
        setRecipients(list.filter((u: any) => u.schoolRole === 'ADMIN' || u.schoolRole === 'PARENT'));
      } catch (err) {
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest">
            Send secure communications to administrators and parent users
          </p>
        </div>
                ...
                >
                  ...
                  {recipients.map((r) => (
                    <option key={r.id} value={r.id}>
                      {r.name || r.username} ({r.schoolRole})
                    </option>
                  ))}
                </select>
              </div>
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-xs font-bold text-slate-400 mt-1">
                      From: <span className="text-slate-800">{selectedMessage.sender?.name || selectedMessage.sender?.username}</span> • To: <span className="text-slate-800">{selectedMessage.recipient?.name || selectedMessage.recipient?.username}</span>
                    </p>
                  </div>
