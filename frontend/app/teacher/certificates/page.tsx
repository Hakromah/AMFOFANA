  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setCerts(res.data || []);
      } catch { toast.error('Unable to load your credentials'); }
    ...
    <div className="p-[clamp(1rem,2vw+1rem,2rem)] space-y-6 bg-slate-50/50 min-h-screen">
      <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }}>
        <h1 className="text-[clamp(1.2rem,2vw+1rem,1.7rem)] font-black text-slate-900 tracking-tighter flex items-center gap-3 italic">
          MY TEACHER CREDENTIALS <Award className="text-amber-500" size={22}/>
        </h1>
        <p className="text-slate-400 font-bold text-[10px] uppercase tracking-[0.3em] mt-0.5">
          Official certificates, service attestations and recommendation letters issued by AMFOFANA High School
        </p>
      </motion.div>
      ...
      <Card className="border border-slate-100 shadow bg-white rounded-3xl overflow-hidden">
        <CardHeader className="bg-slate-900 text-white py-4 px-6">
          <CardTitle className="text-[10px] font-black uppercase tracking-[0.2em] flex items-center gap-2">
            <Award size={13} className="text-amber-400"/> My Official Certificates
          </CardTitle>
        </CardHeader>
          ...
          <Table>
            <TableHeader className="bg-slate-50 sticky top-0">
              <TableRow className="border-slate-100 hover:bg-transparent">
                {['Serial No', 'Type', 'Description / Programme', 'Issue Date', 'Status', 'PDF'].map(h => (
                ...
                {loading ? (
                  ...
                  <TableRow key="empty">
                    <TableCell colSpan={6} className="text-center py-16">
                      ...
                      <p className="text-[11px] text-slate-300 mt-1">Official service certificates will appear here once issued by school management</p>
                    </TableCell>
                  </TableRow>
                ) : (
                  certs.map(cert => (
