...
//       // PULL REAL DATA FROM FIRST RESULT ITEM
//       const studentInfo = results[0]?.student;
//       const studentFullName = studentInfo?.name || "N/A";
//       const studentId = studentInfo?.userId || studentInfo?.id || "N/A";
...
//       doc.setFontSize(18);
//       doc.setFont("helvetica", "bold");
//       doc.text(schoolFullName, 40, 20);

//       doc.setFontSize(9);
//       doc.setFont("helvetica", "normal");
//       doc.setTextColor(100);
//       doc.text("123 Education Drive, Academic City | +1 234 567 890", 40, 26);
//       doc.text("registrar@amf-school.edu", 40, 31);

//       doc.setDrawColor(226, 232, 240);
//       doc.line(14, 38, 196, 38);

//       // 2. REAL STUDENT DATA SECTION
//       doc.setFontSize(12);
//       doc.setTextColor(30, 41, 59);
//       doc.setFont("helvetica", "bold");
//       doc.text("OFFICIAL ACADEMIC TRANSCRIPT", 14, 48);

//       doc.setFontSize(10);
//       doc.setFont("helvetica", "normal");
//       doc.text(`Full Name:`, 14, 58);
//       doc.setFont("helvetica", "bold");
//       doc.text(studentFullName.toUpperCase(), 45, 58); // REAL NAME

//       doc.setFont("helvetica", "normal");
//       doc.text(`Student ID:`, 14, 64);
//       doc.setFont("helvetica", "bold");
//       doc.text(String(studentId), 45, 64); // REAL ID

//       doc.setFont("helvetica", "normal");
//       doc.text(`Issue Date:`, 130, 58);
//       doc.text(`${date}`, 160, 58);
//       doc.text(`Status:`, 130, 64);
//       doc.setTextColor(parseFloat(averageScore) >= 50 ? 22 : 220, parseFloat(averageScore) >= 50 ? 163 : 38, 74);
//       doc.text(`${parseFloat(averageScore) >= 50 ? 'GOOD STANDING' : 'PROBATION'}`, 160, 64);

//       // 3. Performance Summary
//       autoTable(doc, {
//          startY: 75,
//          head: [["Total Exams", "Cumulative Average", "Letter Grade"]],
//          body: [[results.length, `${averageScore}%`, calculateLetterGrade(parseFloat(averageScore))]],
//          theme: 'grid',
//          headStyles: { fillColor: [30, 41, 59] },
//          styles: { halign: 'center', fontSize: 11 }
...
//       doc.setFontSize(12);
//       doc.setFont("helvetica", "bold");
//       doc.text("Examination Details", 14, (doc as any).lastAutoTable.finalY + 15);
...
//       doc.setTextColor(100);
//       doc.line(14, finalY, 70, finalY);
//       doc.text("Director of Academic Records", 14, finalY + 5);
//       doc.text("Generated via Secure Student Portal", 14, doc.internal.pageSize.height - 10);
...
//                <p className="text-muted-foreground">Manage and track your official examination history.</p>
//             </div>
//             <div className="flex items-center gap-4">
//                <Button variant="outline" onClick={downloadTranscript} className="gap-2 border-primary/20 hover:bg-primary/5">
//                   <Download className="w-4 h-4" /> Official Transcript
...
//                <div className="flex items-center gap-4 bg-primary/5 p-4 rounded-xl border border-primary/10">
//                   <div className="text-right">
//                      <p className="text-xs uppercase text-muted-foreground font-bold">Cumulative Average</p>
//                      <p className="text-3xl font-black text-primary">{averageScore}%</p>
//                   </div>
//                   <Award className="w-8 h-8 text-primary" />
//                </div>
...
//                   <div className="flex items-center gap-2">
//                      <TrendingUp className="w-5 h-5 text-blue-500" />
//                      <CardTitle className="text-base font-semibold">Growth Trend vs. Class Average</CardTitle>
//                   </div>
//                </CardHeader>
//                <CardContent className="h-[320px]">
//                   <ResponsiveContainer width="100%" height="100%">
...
//                <Card className="bg-primary text-primary-foreground border-none shadow-lg">
//                   <CardHeader className="pb-2">
//                      <CardTitle className="text-primary-foreground/70 text-xs font-bold uppercase">Best Performance</CardTitle>
//                   </CardHeader>
//                   <CardContent>
//                      <h2 className="text-4xl font-black">{highestScore}%</h2>
//                      <p className="text-sm mt-1 opacity-80 font-medium">Top marks achieved</p>
//                   </CardContent>
//                </Card>

//                <Card className="flex-1 border-dashed">
//                   <CardHeader className="pb-2">
//                      <CardTitle className="text-xs font-bold text-muted-foreground uppercase">Assessment Count</CardTitle>
//                   </CardHeader>
//                   <CardContent className="flex items-center gap-4">
//                      <BookOpen className="w-8 h-8 text-primary/40" />
//                      <h2 className="text-3xl font-bold">{results.length} Exams</h2>
...
//                                  <span className={`text-base font-bold ${!isPassing ? 'text-red-500' : 'text-slate-900'}`}>{r.marks}%</span>
//                               </TableCell>
//                               <TableCell className="text-center text-slate-400 font-medium">{r.classAverage ? `${r.classAverage}%` : '—'}</TableCell>
//                               <TableCell className="text-center">
//                                  <div className="inline-flex items-center justify-center w-9 h-9 rounded-lg bg-primary/10 text-primary font-black text-xs">
//                                     {r.grade || calculateLetterGrade(r.marks)}
//                                  </div>
...
//                         <TableRow>
//                            <TableCell colSpan={6} className="h-32 text-center text-muted-foreground">
//                               Your records are being finalized by the school administration.
//                            </TableCell>
//                         </TableRow>
//                      )}
//                   </TableBody>
