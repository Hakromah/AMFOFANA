...
export default function StudentTimetablePage() {
  ...
  const handleExportPDF = () => {
    ...
    const studentClassName = timetable[0]?.classe?.name || 'Student Timetable';
    try {
      const pdfData = {
      ...
      toast.success('Official timetable downloaded');
    } catch (err) {
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            <Badge className="bg-blue-600/10 text-blue-600 border-none px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest">
              Student Portal • {enrolledClassName ? ('Class: ' + enrolledClassName) : 'Enrolled'}
            </Badge>
          </div>
          <h1 className="text-[clamp(1.5rem,2.5vw+1rem,2.8rem)] font-black text-slate-950 tracking-tighter">
            My Class <span className="text-blue-600 italic">Timetable</span>
          </h1>
        </div>
          ...
          {currentClass ? (
            ...
            <div className="py-2">
              ...
              <p className="text-xs text-slate-400 font-bold mt-1">
                Enjoy your break or consult your next scheduled period.
              </p>
            </div>
          )}
        </div>
            ...
            {nextClass ? (
              ...
              <div className="py-2">
                <h3 className="text-base font-black text-slate-600">
                  End of scheduled classes today
                </h3>
                <p className="text-xs text-slate-400 font-bold mt-1">
                  All classes completed for the day.
                </p>
              </div>
            )}
          </div>
                      ...
                      <div className="p-5 flex-1 flex flex-col justify-between">
                        ...
                        {entry.teacher && (
                          <p className="text-xs font-bold text-slate-500 mt-2 flex items-center gap-1.5">
                            ...
                            Instructor: {entry.teacher.name || entry.teacher.username}
                          </p>
                        )}
                      </div>
                    </div>
