  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        toast.error('Unable to load your certificates'); 
      } finally { 
        setLoading(false); 
      }
    };
    ...
    <div className="p-[clamp(1rem,2vw+1rem,2rem)] space-y-6 bg-slate-50/50 min-h-screen">
      <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }}>
        <h1 className="text-[clamp(1.2rem,2vw+1rem,1.7rem)] font-black text-slate-900 tracking-tighter flex items-center gap-3 italic">
          MY CERTIFICATES &amp; DIPLOMAS <Award className="text-amber-500" size={22} />
        </h1>
        <p className="text-slate-400 font-bold text-[10px] uppercase tracking-[0.3em] mt-0.5">
          Official academic credentials and certificates issued by AMFOFANA High School
        </p>
      </motion.div>
        ...
        <Card className="border border-slate-100 shadow bg-white rounded-3xl overflow-hidden">
          <CardHeader className="bg-slate-900 text-white py-4 px-6">
            <CardTitle className="text-[10px] font-black uppercase tracking-[0.2em] flex items-center gap-2">
              <Award size={13} className="text-amber-400"/> My Official Credentials
            </CardTitle>
          </CardHeader>
                  ...
                  {loading ? (
                    ...
                    <TableRow key="empty">
                      <TableCell colSpan={6} className="text-center py-16">
                        ...
                        <p className="text-xs font-black uppercase text-slate-400">No certificates available yet</p>
                        <p className="text-[11px] text-slate-300 mt-1">Official diplomas and certificates will appear here once issued by administration</p>
                      </TableCell>
                    </TableRow>
                  ) : (
                    certs.map(cert => (
                      <motion.tr key={cert.id} initial={{ opacity:0 }} animate={{ opacity:1 }} className="hover:bg-slate-50/80 border-slate-50">
                        ...
                        <TableCell>
                          {cert.mention ? <div><span className="text-[11px] font-black text-amber-600 block">{cert.mention}</span>{cert.gpa != null && <span className="text-[10px] text-emerald-600 font-mono">{Number(cert.gpa).toFixed(2)}/{(cert.maxGpa||4).toFixed(1)}</span>}</div> : <span className="text-slate-300">—</span>}
                        </TableCell>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-1.5">
                            ...
                            <Button variant="ghost" size="icon" className="h-8 w-8 text-blue-500 hover:bg-blue-50" title="Digital Security Hash"
