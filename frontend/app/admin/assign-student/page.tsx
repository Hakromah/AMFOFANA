...
export default function AdvancedAssignStudentPage() {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || "Enrollment rejected by the system.";
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                        ...
                        <Select onValueChange={field.onChange} value={field.value}>
                          <FormControl>
                            <SelectTrigger className="h-16 rounded-2xl bg-slate-50 border-none font-bold text-slate-700 shadow-sm transition-all focus:ring-2 focus:ring-blue-600/20">
                              <SelectValue placeholder="Search in student registry..." />
                            </SelectTrigger>
                          </FormControl>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Select Target Class</FormLabel>
                        <Select onValueChange={field.onChange} value={field.value}>
                          <FormControl>
          ...
          <div className="bg-white rounded-[clamp(1rem,2vw+1rem,2rem)] p-[clamp(0rem,2vw+1rem,2rem)] border border-slate-100 md:hover:border-primary duration-500 transition-colors shadow-sm flex-1 flex flex-col justify-between">
            <div className="space-y-[clamp(0rem,2vw+1rem,2rem)]">
              <div className="flex items-center justify-between">
                <h2 className="text-[clamp(1rem,3vw,4rem)]  font-black text-slate-900 italic tracking-tight uppercase leading-none">Intelligence Center</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 space-y-2 group md:hover:bg-primary transition-all duration-500">
                  ...
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest group-hover:text-blue-100 transition-colors">Study Groups</p>
                </div>
              </div>
