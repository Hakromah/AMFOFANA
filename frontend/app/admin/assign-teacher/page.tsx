  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] p-[clamp(1rem,2vw+1rem,2rem)] space-y-[clamp(1rem,2vw+1rem,2rem)]">
      ...
      <header className="max-w-6xl mx-auto text-center space-y-4">
        ...
        >
          <ShieldCheck size={18} />
          <span className="text-[10px] font-black uppercase tracking-[0.4em]">Registration Authority</span>
        </motion.div>
        <h1 className="text-[clamp(1.4rem,3.5vw,4rem)] font-black text-slate-900 tracking-tighter sm:text-7xl italic uppercase">
          Teacher <span className="text-primary">Assignment.</span>
        </h1>
        <p className="text-slate-400 font-bold text-sm uppercase tracking-widest max-w-xl mx-auto leading-loose">
          Establish links between teaching staff and class registers to authorize exam management.
        </p>
      </header>
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Choose a Faculty Member</FormLabel>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Target Class</FormLabel>
                        <Select onValueChange={field.onChange} value={field.value}>
                          <FormControl>
          ...
          <div className="bg-white rounded-[clamp(1rem,2vw+1rem,2rem)] p-[clamp(0rem,2vw+1rem,2rem)] border border-slate-100 md:hover:border-primary duration-500 transition-colors shadow-sm h-full flex flex-col justify-between">
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h2 className="text-[clamp(16px,3vw,24px)] font-black text-slate-900 italic tracking-tight uppercase">Quick Info</h2>
              ...
              <div className="space-y-4">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] px-2">Assignment Impact</p>
                <ul className="space-y-3">
                  {[
                    "Grant exam management permissions",
                    "Enable marks entry for designated classes",
                    "Provide attendance tracking authority",
                    "Synchronize student timetables with the instructor"
                  ].map((text, i) => (
