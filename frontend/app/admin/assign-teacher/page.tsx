...
export default function AdvancedAssignTeacherPage() {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    if (duplicate) {
      const teacherName = selectedTeacher ? getDisplayName(selectedTeacher) : 'This teacher';
      toast.warning('Teacher Already Assigned', {
        description: `${teacherName} is already assigned to "${duplicate.name}".`,
      });
      return;
    }

    setIsLoading(true);
    const tid = toast.loading('Processing assignment, please wait...');
    try {
      ...
      if (msg?.toLowerCase().includes('already')) {
        toast.warning('Assignment Already Exists', { id: tid, description: msg });
      } else {
        toast.error('Assignment failed', { id: tid, description: msg });
      }
    } finally {
      setIsLoading(false);
    }
              ...
              {selectedTeacher ? (
                ...
                <div className="p-3.5 bg-amber-50/60 border border-amber-200/60 rounded-2xl flex items-center gap-3">
                  ...
                  <p className="text-xs font-bold text-amber-700">
                    Please select a teacher from the right panel.
                  </p>
                </div>
              )}
                        ...
                        >
                          <FormControl>
                            <SelectTrigger className="h-14 rounded-2xl bg-slate-50 border-none font-bold text-slate-700 shadow-sm transition-all focus:ring-2 focus:ring-blue-600/20 disabled:opacity-50">
                              <SelectValue placeholder={selectedTeacher ? 'Choose a target class...' : 'Select a teacher first'} />
                            </SelectTrigger>
                          </FormControl>
              ...
              <div className="flex flex-col items-center justify-center py-14 gap-3 bg-slate-50/70 rounded-3xl border border-dashed border-slate-200">
                ...
                <div className="text-center">
                  ...
                  <p className="text-[10px] text-slate-400 font-bold mt-0.5">
                    Select a teacher above to display their assigned classes
                  </p>
                </div>
              </div>
            ) : (
                  ...
                  <div className="flex flex-col items-center justify-center py-10 gap-3 bg-slate-50 rounded-3xl border border-dashed border-slate-200">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-[10px] text-slate-300 font-bold mt-1">
                        Use the form on the left to assign a class
                      </p>
                    </div>
                  </div>
                ) : (
                              ...
                              <TooltipTrigger asChild>
                                <AlertDialogTrigger asChild>
                                  <button
                                    ...
                                    aria-label={`Remove ${cls.name} from teacher ${teacherDisplayName}`}
                            ...
                            <AlertDialogContent className="rounded-3xl">
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription className="text-slate-500">
                                  ...
                                  <strong className="text-slate-800">{cls.name}</strong> from{' '}
                                  <strong className="text-slate-800">{teacherDisplayName}</strong>?
                                  <br />
                                  <span className="text-xs text-slate-400 mt-1 inline-block">
                                    This action can be undone by re-assigning the class.
                                  </span>
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
