...
const LESSON_TYPES: Record<string, { label: string; color: string; bg: string; border: string }> = {
  REGULAR: { label: "Lecture / Regular", color: 'text-blue-700', bg: 'bg-blue-50', border: 'border-blue-200' },
  PRACTICAL: { label: "Practical (TP)", color: 'text-emerald-700', bg: 'bg-emerald-50', border: 'border-emerald-200' },
  LAB: { label: "Laboratory", color: 'text-teal-700', bg: 'bg-teal-50', border: 'border-teal-200' },
  TUTORIAL: { label: "Tutorial (TD)", color: 'text-indigo-700', bg: 'bg-indigo-50', border: 'border-indigo-200' },
  BREAK: { label: "Break / Recess", color: 'text-amber-700', bg: 'bg-amber-50', border: 'border-amber-200' },
  LUNCH: { label: "Lunch Break", color: 'text-orange-700', bg: 'bg-orange-50', border: 'border-orange-200' },
  ASSEMBLY: { label: "Assembly / Gathering", color: 'text-purple-700', bg: 'bg-purple-50', border: 'border-purple-200' },
  EXAM: { label: "Exam / Assessment", color: 'text-rose-700', bg: 'bg-rose-50', border: 'border-rose-200' },
  STUDY: { label: "Study Hall", color: 'text-sky-700', bg: 'bg-sky-50', border: 'border-sky-200' },
  SPORTS: { label: "Physical Education / Sports", color: 'text-lime-700', bg: 'bg-lime-50', border: 'border-lime-200' },
  OTHER: { label: "Other Activity", color: 'text-slate-700', bg: 'bg-slate-50', border: 'border-slate-200' },
};

// Standard School Period Presets
const STANDARD_SLOT_PRESETS = [
  { name: "P1 Morning (08:00 - 09:30)", start: '08:00', end: '09:30', periodName: 'Période 1' },
  { name: "P2 Morning (09:45 - 11:00)", start: '09:45', end: '11:00', periodName: 'Période 2' },
  { name: "P3 Midday (11:15 - 12:30)", start: '11:15', end: '12:30', periodName: 'Période 3' },
  { name: "P4 Afternoon (13:00 - 14:30)", start: '13:00', end: '14:30', periodName: 'Période 4' },
  { name: "P5 Afternoon (14:45 - 16:00)", start: '14:45', end: '16:00', periodName: 'Période 5' },
  { name: "P6 Late (16:15 - 17:30)", start: '16:15', end: '17:30', periodName: 'Période 6' },
];
...
const entrySchema = z.object({
  ...
  lessonType: z.string().min(1, "Lesson type required"),
...
export default function TimetableManagement() {
  ...
  useEffect(() => {
    ...
    for (const item of timetable) {
      ...
      if (watchedTeacher && item.teacher?.id && Number(watchedTeacher) === Number(item.teacher.id)) {
        ...
        conflicts.push({
          type: 'TEACHER',
          message: "Teacher " + teacherName + " is already teaching " + subName + ' (' + className + ') ' + "from " + (item.startTime?.substring(0,5)) + " to " + (item.endTime?.substring(0,5)) + '.',
        });
      }
      ...
      if (watchedRoom && item.room?.id && Number(watchedRoom) === Number(item.room.id)) {
        ...
        conflicts.push({
          type: 'ROOM',
          message: "Room " + rName + " is already booked by " + className + ' ' + "from " + (item.startTime?.substring(0,5)) + " to " + (item.endTime?.substring(0,5)) + '.',
        });
      }
      ...
      if (watchedClass && item.classe?.id && Number(watchedClass) === Number(item.classe.id)) {
        ...
        conflicts.push({
          type: 'CLASS',
          message: "Class " + className + " already has " + subName + " scheduled from " + (item.startTime?.substring(0,5)) + " to " + (item.endTime?.substring(0,5)) + '.',
        });
      }
    }
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      toast.error("Failed to sync timetable data");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const onSubmitEntry = async (values: z.infer<typeof entrySchema>) => {
    ...
    try {
      ...
      toast.error("Conflict or error detected: " + msg);
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDeleteEntry = async (id: number) => {
    if (!confirm("Are you sure you want to remove this session from schedule?")) return;
    try {
  ...
  const handleRunDuplication = async () => {
    ...
    try {
      if (dupMode === 'DAY') {
        ...
        toast.success(res.data?.message || "Day schedule duplicated");
      } else if (dupMode === 'CLASS') {
        if (!dupSourceClass || !dupTargetClass) {
          toast.error("Please select both source and target class");
          return;
        }
        ...
        toast.success(res.data?.message || "Class schedule duplicated");
      } else if (dupMode === 'TERM') {
        if (!dupSourceSem || !dupTargetSem) {
          toast.error("Please select both source and target semester");
          return;
        }
        ...
        toast.success(res.data?.message || "Semester schedule duplicated");
      }
      setIsDuplicateDialogOpen(false);
      fetchData();
    } catch (err: any) {
      toast.error(err?.response?.data?.error?.message || "Duplication failed");
    } finally {
  ...
  const handleExportPDF = () => {
    if (filteredTimetable.length === 0) {
      toast.error("No timetable entries found for PDF export");
      return;
    }
    const currentClass = classes.find(c => String(c.id) === selectedClassId);
    const className = currentClass ? currentClass.name : "General Timetable";
    ...
    <div className="min-h-screen bg-[#F8FAFC] p-6 lg:p-10 space-y-8">
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            <Badge className="bg-blue-600/10 text-blue-600 border-none px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5">
              <Sparkles size={12} /> High School Timetable System
            </Badge>
            {conflictCount > 0 && (
              <Badge className="bg-rose-600 text-white border-none px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-1 animate-pulse">
                <AlertTriangle size={12} /> {conflictCount} Conflicts Detected
              </Badge>
            )}
          </div>
          <h1 className="text-3xl lg:text-4xl font-black text-slate-950 tracking-tight">
            Timetable Management Hub
          </h1>
          <p className="text-xs text-slate-500 font-medium mt-1">
            Plan, conflict-check, and publish school schedules across classes, teachers, and venues
          </p>
        </div>
      ...
      <div className="max-w-7xl mx-auto bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex flex-wrap items-center justify-between gap-4">
        <div className="flex flex-wrap items-center gap-3 flex-1">
          ...
          <div className="min-w-44">
            ...
            <Select value={selectedYearId} onValueChange={setSelectedYearId}>
              ...
              <SelectContent className="rounded-xl">
                <SelectItem value="all" className="font-bold text-xs">All academic years</SelectItem>
                {academicYears.map((y) => (
          ...
          <div className="min-w-40">
            ...
            <Select value={selectedSemesterId} onValueChange={setSelectedSemesterId}>
              ...
              <SelectContent className="rounded-xl">
                <SelectItem value="all" className="font-bold text-xs">All semesters</SelectItem>
                {semesters.map((s) => (
          ...
          <div className="min-w-40">
            ...
            <Select value={selectedClassId} onValueChange={setSelectedClassId}>
              ...
              <SelectContent className="rounded-xl">
                <SelectItem value="all" className="font-bold text-xs">All classes</SelectItem>
                {classes.map((c) => (
          ...
          <div className="min-w-40">
            ...
            <Select value={selectedTeacherId} onValueChange={setSelectedTeacherId}>
              ...
              <SelectContent className="rounded-xl">
                <SelectItem value="all" className="font-bold text-xs">All teachers</SelectItem>
                {teachers.map((tch) => (
          ...
          <div className="min-w-36">
            ...
            <Select value={selectedRoomId} onValueChange={setSelectedRoomId}>
              ...
              <SelectContent className="rounded-xl">
                <SelectItem value="all" className="font-bold text-xs">All rooms</SelectItem>
                {rooms.map((r) => (
        ...
        <div className="flex items-center gap-3 bg-slate-50 p-2.5 rounded-2xl border border-slate-100">
          ...
          <div className="h-7 w-px bg-slate-200" />
          <div className="text-center px-2.5">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Rooms</p>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="bg-white p-1.5 rounded-2xl border border-slate-100 shadow-sm flex flex-wrap gap-1 h-auto">
            ...
            >
              <Grid size={15} /> Class Schedule
            </TabsTrigger>
            <TabsTrigger
              value="teachers"
              className="rounded-xl font-black text-xs uppercase tracking-wider py-2.5 px-5 data-[state=active]:bg-slate-950 data-[state=active]:text-white flex items-center gap-2"
            >
              <Users size={15} /> Teacher Master
            </TabsTrigger>
            <TabsTrigger
              value="rooms"
              className="rounded-xl font-black text-xs uppercase tracking-wider py-2.5 px-5 data-[state=active]:bg-slate-950 data-[state=active]:text-white flex items-center gap-2"
            >
              <Building2 size={15} /> Room Occupancy
            </TabsTrigger>
            <TabsTrigger
              value="conflicts"
            ...
            >
              <AlertTriangle size={15} /> Conflicts ({conflictCount})
            </TabsTrigger>
            <TabsTrigger
              value="config"
              className="rounded-xl font-black text-xs uppercase tracking-wider py-2.5 px-5 data-[state=active]:bg-slate-950 data-[state=active]:text-white flex items-center gap-2"
            >
              <Sliders size={15} /> Rooms & Slots
            </TabsTrigger>
            <TabsTrigger
              value="analytics"
              className="rounded-xl font-black text-xs uppercase tracking-wider py-2.5 px-5 data-[state=active]:bg-slate-950 data-[state=active]:text-white flex items-center gap-2"
            >
              <BarChart3 size={15} /> Analytics & Workload
            </TabsTrigger>
          </TabsList>
                  ...
                  <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-12 text-center flex flex-col items-center justify-center gap-4">
                    ...
                    <div>
                      <h3 className="font-black text-slate-900 text-lg">
                        No classes scheduled for this day ({DAY_LABELS[selectedDay]})
                      </h3>
                      <p className="text-xs text-slate-500 mt-1 max-w-md">
                        {selectedClassName ? ("No teaching sessions are recorded for this day matching your current filters." + ' (' + selectedClassName + ')') : "No teaching sessions are recorded for this day matching your current filters."}
                      </p>
                    </div>
                    <Button
                    ...
                    >
                      <Plus size={16} className="mr-2" /> Schedule a Class ({DAY_LABELS[selectedDay]})
                    </Button>
                  </div>
                ) : (
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between px-2">
                      <p className="text-xs font-black text-slate-500 uppercase tracking-widest">
                        {dayEntries.length} Session(s) on {DAY_LABELS[selectedDay]} {selectedClassName ? ('• ' + selectedClassName) : ''}
                      </p>
                      <Button
                        onClick={() => handleQuickAdd(selectedDay)}
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-black text-slate-900">
                    Timetable Audit & Conflict Detection Engine
                  </h3>
                  <p className="text-xs text-slate-500">
                    Automatic detection of teacher overlaps, room collisions, and class double-bookings
                  </p>
                </div>
                <Badge className={conflictCount === 0 ? 'bg-emerald-600 text-white font-black text-xs px-3 py-1.5' : 'bg-rose-600 text-white font-black text-xs px-3 py-1.5'}>
                  {conflictCount === 0 ? "0 Conflicts — Clean Schedule" : (conflictCount + ' ' + "Active Conflict(s)")}
                </Badge>
              </div>
              ...
              {conflictCount === 0 ? (
                <div className="text-center py-12 flex flex-col items-center justify-center space-y-3">
                  ...
                  <h4 className="text-base font-black text-slate-900">
                    No scheduling conflicts found across the entire timetable
                  </h4>
                  <p className="text-xs text-slate-500 max-w-md">
                    All teachers, venues, and student groups have distinct, non-overlapping teaching periods.
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
          ...
          <TabsContent value="config" className="space-y-6 mt-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-black text-slate-900 text-sm uppercase">Classrooms & Venues</h3>
                    <p className="text-xs text-slate-400">Manage rooms and capacities</p>
                  </div>
                  <Button
                    onClick={() => {
                      setEditingRoom(null);
                ...
                <div className="space-y-2 max-h-96 overflow-y-auto pr-1">
                  ...
                  {rooms.length === 0 && (
                    <p className="text-xs text-slate-400 text-center py-6 italic">No rooms configured</p>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-black text-slate-900 text-sm uppercase">Standard Time Slots</h3>
                    <p className="text-xs text-slate-400">Define period schedule template</p>
                  </div>
                  <Button
                    onClick={() => {
                      setEditingSlot(null);
                ...
                <div className="space-y-2 max-h-96 overflow-y-auto pr-1">
                  ...
                  {timeSlots.length === 0 && (
                    <p className="text-xs text-slate-400 text-center py-6 italic">No time slots configured</p>
                  )}
                </div>
              </div>
            </div>
          ...
          <TabsContent value="analytics" className="space-y-6 mt-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 space-y-4">
                <h3 className="font-black text-slate-900 text-sm uppercase">Teacher Workload</h3>
                <div className="space-y-3">
              ...
              <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 space-y-4">
                <h3 className="font-black text-slate-900 text-sm uppercase">Room Utilization</h3>
                <div className="space-y-3">
              ...
              <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 space-y-4">
                <h3 className="font-black text-slate-900 text-sm uppercase">Subjects by Class</h3>
        ...
        <DialogContent className="rounded-[2.5rem] border-none shadow-2xl p-8 max-w-2xl max-h-[92vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-2xl font-black italic tracking-tight uppercase">
              {editingEntry ? "Edit" : "New"} <span className="text-blue-600">Class Session</span>
            </DialogTitle>
            <DialogDescription className="text-xs text-slate-500 font-medium">
              Configure class, subject, instructor, venue, and class time.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-2.5">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-black uppercase tracking-wider text-slate-600 flex items-center gap-1.5">
                <Clock size={13} className="text-blue-600" /> Quick-Pick Time Slot Presets
              </span>
              <span className="text-[10px] text-slate-500 font-bold">
                Click to fill start & end times
              </span>
            </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="font-bold">
                    Early morning time detected: <span className="font-mono">{watchedStart}</span> (1:00 AM in the night in 24h format)
                  </p>
                  <p className="text-[11px] text-amber-700">
                    Did you mean afternoon class at <strong className="font-mono">{convertedPMTime}</strong> (01:00 PM) ?
                  </p>
                </div>
              </div>
              <Button
          ...
          {liveConflicts.length > 0 && (
            <div className="p-4 rounded-2xl bg-rose-50 border border-rose-200 text-rose-800 text-xs space-y-2">
              <div className="flex items-center gap-1.5 font-black text-[11px] uppercase tracking-wider text-rose-700">
                <AlertTriangle size={15} /> Conflict Detected Alert
              </div>
              ...
              <div className="pt-2 border-t border-rose-200/80 flex items-center gap-2">
                ...
                <label htmlFor="forceOverrideCheck" className="text-xs font-bold text-rose-900 cursor-pointer">
                  Ignore warning and force save session (Admin Override)
                </label>
              </div>
            </div>
          )}
            ...
            <form onSubmit={form.handleSubmit(onSubmitEntry)} className="space-y-4 mt-2">
              ...
              <div className="flex items-center justify-between px-3 py-2 rounded-xl bg-slate-100/70 border border-slate-200 text-xs">
                <span className="text-slate-500 font-bold flex items-center gap-1.5">
                  <Clock size={13} className="text-slate-600" /> Calculated Duration:
                </span>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Period 1, Morning..." {...field} className="rounded-xl font-bold text-xs" />
                      </FormControl>
                    </FormItem>
                  )}
                />
        ...
        <DialogContent className="rounded-[2.5rem] border-none shadow-2xl p-8 max-w-md">
          <DialogHeader>
            <DialogTitle className="text-xl font-black italic tracking-tight uppercase">
              Session <span className="text-blue-600">Details</span>
            </DialogTitle>
            <DialogDescription className="text-xs text-slate-500 font-medium">
              Scheduled teaching session overview
            </DialogDescription>
          </DialogHeader>
          {selectedEntry && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <Badge className={'text-[10px] font-black uppercase tracking-wider ' + (LESSON_TYPES[selectedEntry.lessonType]?.bg || 'bg-blue-50') + ' ' + (LESSON_TYPES[selectedEntry.lessonType]?.color || 'text-blue-700') + ' border'}>
                  {LESSON_TYPES[selectedEntry.lessonType]?.label || "Class"}
                </Badge>
              ...
              <div>
                <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight">
                  {selectedEntry.subject?.name || "Subject"}
                </h3>
              ...
              <div className="space-y-3 bg-slate-50 p-4 rounded-2xl border border-slate-100 text-xs">
                <div className="flex items-center justify-between">
                  <span className="text-slate-500 font-bold">Room / Venue</span>
                  <span className="font-black text-slate-800">{selectedEntry.room?.name || selectedEntry.roomName || "Unassigned"}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-500 font-bold">Teacher</span>
                  <span className="font-black text-slate-800">{selectedEntry.teacher ? (selectedEntry.teacher.name || selectedEntry.teacher.username) : "Unassigned"}</span>
                </div>
        ...
        <DialogContent className="rounded-[2.5rem] border-none shadow-2xl p-8 max-w-lg">
          <DialogHeader>
            <DialogTitle className="text-2xl font-black italic tracking-tighter uppercase">
              Batch Duplication <span className="text-blue-600">Tools</span>
            </DialogTitle>
            <DialogDescription className="text-xs text-slate-500 font-medium">
              Duplicate full days, copy grade schedules, or rollover academic semesters.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-1.5">
                  Target Days (to receive classes)
                </label>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Select value={dupSourceClass} onValueChange={setDupSourceClass}>
                  <SelectTrigger className="rounded-xl font-bold text-xs"><SelectValue placeholder="Choose Class" /></SelectTrigger>
                  <SelectContent className="rounded-xl">
            ...
            <div className="space-y-4">
              <div>
                ...
                <Select value={dupSourceSem} onValueChange={setDupSourceSem}>
                  <SelectTrigger className="rounded-xl font-bold text-xs"><SelectValue placeholder="All semesters" /></SelectTrigger>
                  <SelectContent className="rounded-xl">
      ...
      <Dialog open={isRoomDialogOpen} onOpenChange={setIsRoomDialogOpen}>
        <DialogContent className="rounded-[2.5rem] border-none shadow-2xl p-8 max-w-md">
          <DialogHeader>
            <DialogTitle className="text-2xl font-black italic tracking-tighter uppercase">
              {editingRoom ? "Edit" : "New"} <span className="text-blue-600">Classroom Venue</span>
            </DialogTitle>
          </DialogHeader>
          ...
          <form onSubmit={handleSaveRoom} className="space-y-4 mt-2">
            ...
            <div className="flex gap-3 pt-2">
              <Button type="button" variant="outline" onClick={() => setIsRoomDialogOpen(false)} className="flex-1 rounded-2xl h-11 font-black text-xs">Cancel</Button>
              <Button type="submit" className="flex-1 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl h-11 font-black text-xs">Save Session</Button>
            </div>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isSlotDialogOpen} onOpenChange={setIsSlotDialogOpen}>
        <DialogContent className="rounded-[2.5rem] border-none shadow-2xl p-8 max-w-md">
          <DialogHeader>
            <DialogTitle className="text-2xl font-black italic tracking-tighter uppercase">
              {editingSlot ? "Edit" : "New"} <span className="text-purple-600">Time Slot</span>
            </DialogTitle>
          </DialogHeader>
          ...
          <form onSubmit={handleSaveSlot} className="space-y-4 mt-2">
            ...
            <div>
              ...
              <Select value={slotType} onValueChange={setSlotType}>
                <SelectTrigger className="rounded-xl font-bold text-xs"><SelectValue /></SelectTrigger>
                <SelectContent className="rounded-xl">
                  <SelectItem value="LESSON" className="font-bold text-xs">Regular Lesson</SelectItem>
                  <SelectItem value="BREAK" className="font-bold text-xs">Break / Recess</SelectItem>
                  <SelectItem value="LUNCH" className="font-bold text-xs">Lunch Break</SelectItem>
                  <SelectItem value="ASSEMBLY" className="font-bold text-xs">Assembly</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="flex gap-3 pt-2">
              <Button type="button" variant="outline" onClick={() => setIsSlotDialogOpen(false)} className="flex-1 rounded-2xl h-11 font-black text-xs">Cancel</Button>
              <Button type="submit" className="flex-1 bg-purple-600 hover:bg-purple-700 text-white rounded-2xl h-11 font-black text-xs">Save Session</Button>
            </div>
          </form>
        </DialogContent>
      </Dialog>
