...
export default function StudentFinance() {
      ...
      try {
        ...
        if (activeAssignment && activeAssignment.transportFee > 0) {
          setChargeItems(prev => {
            ...
            return [...filtered, {
              description: `Transport fee (${activeAssignment.routeName})`,
  ...
  const handleCreateOrEditInvoice = async () => {
    ...
    if (subtotal <= 0) {
      toast.error('Invoice breakdown amounts must be greater than 0');
      return;
    }
  ...
  const handleSubmitSelectedInvoices = async () => {
    ...
    try {
      ...
      toast.success('Invoices submitted successfully', { id: tid });
      setSelectedInvoiceIds([]);
      fetchAllData();
    } catch (e) {
      toast.error('Failed to submit invoices', { id: tid });
    }
  };
  ...
  const handleSubmitSelectedPayments = async () => {
    ...
    try {
      ...
      toast.success('Payments submitted successfully', { id: tid });
      setSelectedPaymentIds([]);
      fetchAllData();
    } catch (e) {
      toast.error('Failed to submit payments', { id: tid });
    }
  };
  ...
  const handleRejectSubmit = async () => {
    ...
    const tid = toast.loading('Recording rejection...');
    try {
  ...
  const downloadReceipt = async (paymentDocId: string, paymentId: number) => {
    ...
    try {
      ...
      if (!pay) {
        toast.error('No payment record found', { id: tid });
        return;
      }
      ...
      doc.text('SCHOOL PAYMENT RECEIPT', 52, 30);
      doc.text('Conakry, Guinea | billing@amfofana.edu', 52, 36);

      doc.setTextColor(15, 23, 42);
      doc.setFontSize(20);
      doc.setFont('Helvetica', 'bold');
      doc.text('SCHOOL PAYMENT RECEIPT', 15, 70);
      doc.setFontSize(10);
      doc.setFont('Helvetica', 'normal');
      ...
      toast.success('Receipt PDF downloaded successfully', { id: tid });
    } catch (e: any) {
      console.error('Receipt error:', e);
      toast.error('Failed to generate receipt PDF', { id: tid });
    }
  };

  const downloadStatement = async (studentId: number | null) => {
    if (!studentId) {
      toast.error('Statement Export: Invalid student ID');
      return;
    }
    const tid = toast.loading('Compiling statement ledger...');
    try {
      ...
      doc.setFontSize(11);
      doc.setFont('Helvetica', 'bold');
      doc.text('Account Ledger Activity (Chronological)', 15, 96);
      ...
      doc.setFontSize(7);
      doc.setTextColor(148, 163, 184);
      doc.text('Scan to verify account statement', 155, 286);

      doc.save(`Account-Statement-${studentUserId || studentId}.pdf`);
      toast.success('Account statement generated successfully', { id: tid });
    } catch (e: any) {
      toast.error('Failed to generate account statement', { id: tid });
      console.error(e);
    }
  };
    ...
    <div className="p-8 space-y-8 bg-slate-50/50 min-h-screen">
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          <h1 className="text-3xl font-black tracking-tight text-slate-900 italic uppercase">Students' Financial Ledger</h1>
        ...
        <div className="flex items-center gap-3">
          {role === 'ACCOUNTANT' && selectedInvoiceIds.length > 0 && (
            ...
            >
              Submit Selection ({selectedInvoiceIds.length})
            </Button>
          )}
          ...
          {(role === 'ACCOUNTLEAD' || role === 'ADMIN') && selectedInvoiceIds.length > 0 && (
            ...
            >
              Approve Selection ({selectedInvoiceIds.length})
            </Button>
          )}
      ...
      <Card className="border-0 shadow-xl shadow-slate-100/50 bg-white rounded-3xl overflow-hidden">
        <CardHeader className="px-6 py-5 border-b border-slate-50">
          <CardTitle className="text-sm font-black uppercase tracking-wider text-slate-500">Student Finance Ledger</CardTitle>
        </CardHeader>
        ...
        <Card className="border-0 shadow-xl shadow-slate-100/50 bg-white rounded-3xl overflow-hidden">
          <CardHeader className="px-6 py-5 border-b border-slate-50">
            <CardTitle className="text-sm font-black uppercase tracking-wider text-slate-500 flex items-center gap-2">
              <Clock className="w-4 h-4 text-amber-500" /> Payments Pending Review
            </CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            <Table>
              ...
              <TableBody>
                {payments.filter(p => p.status === 'SUBMITTED' || p.status === 'REJECTED').length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={7} className="text-center p-6 text-slate-400 font-bold uppercase text-[10px]">No payments pending review found</TableCell>
                  </TableRow>
      ...
      <Card className="border-0 shadow-xl shadow-slate-100/50 bg-white rounded-3xl overflow-hidden">
        <CardHeader className="px-6 py-5 border-b border-slate-50 flex items-center justify-between">
          <CardTitle className="text-sm font-black uppercase tracking-wider text-slate-500">Payments & Receipts History</CardTitle>
          <div className="flex gap-2">
            {role === 'ACCOUNTANT' && selectedPaymentIds.length > 0 && (
              ...
              >
                Submit ({selectedPaymentIds.length})
              </Button>
            )}
            {(role === 'ACCOUNTLEAD' || role === 'ADMIN') && selectedPaymentIds.length > 0 && (
              <Button
                ...
                className="bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold uppercase tracking-wider text-[10px] h-8 px-4"
              >
                Approve ({selectedPaymentIds.length})
              </Button>
            )}
          </div>
        </CardHeader>
      ...
      <Dialog open={isInvoiceOpen} onOpenChange={(open) => { setIsInvoiceOpen(open); if (!open) { setEditingInvoice(null); setSelectedStudentId(''); setInvoiceClassId('NONE'); } }}>
        <DialogContent className="max-w-md bg-white rounded-3xl">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <label className="text-xs font-black uppercase text-slate-400">Fee Details</label>
                <Button
            ...
            <div className="space-y-1">
              <label className="text-xs font-black uppercase text-slate-400">Additional Notes</label>
              <Input
                value={invoiceNotes}
                onChange={(e) => setInvoiceNotes(e.target.value)}
                placeholder="Terms, banking guidelines, custom notes..."
                className="h-11 rounded-xl bg-slate-50"
              />
            </div>
          </div>
          <DialogFooter>
            ...
            >
              {editingInvoice ? 'Save' : 'Generate Invoice'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isPaymentOpen} onOpenChange={(open) => { setIsPaymentOpen(open); if (!open) { setEditingPayment(null); setSelectedInvoiceId(''); setPaymentClassId('NONE'); } }}>
        <DialogContent className="max-w-md bg-white rounded-3xl">
          <DialogHeader>
            <DialogTitle className="text-lg font-black uppercase tracking-wide">
              {editingPayment ? 'Edit Payment Receipt' : 'Receive Payment'}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-1">
            ...
            <div className="space-y-1">
              <label className="text-xs font-black uppercase text-slate-400">Select Invoice</label>
              <Select
          ...
          <DialogFooter>
            ...
            >
              {editingPayment ? 'Save Changes' : 'Submit Payment Receipt'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isRejectOpen} onOpenChange={setIsRejectOpen}>
        <DialogContent className="max-w-sm bg-white rounded-3xl">
          <DialogHeader>
            <DialogTitle className="text-md font-black uppercase tracking-wide text-rose-600">Reject {rejectType}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-3">
            <div className="space-y-1">
              <label className="text-xs font-black uppercase text-slate-400">Specify rejection reason</label>
              <Input
