...
export default function FinanceDashboard() {
  ...
  const fetchStats = useCallback(async (year?: string) => {
    ...
    try {
      ...
      toast.error('Failed to sync financial analysis ledger');
      console.error(e);
    } finally {
      setLoading(false);
    }
  ...
  const kpis = [
    ...
    },
    {
      ...
      value: `${paidRatio}%`,
      desc: `${fmtGNF(stats?.totalCollected || 0)} GNF collected out of ${fmtGNF(stats?.totalInvoiced || 0)} GNF billed`,
      icon: Percent,
      color: paidRatio >= 75
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[80vh] gap-4">
        ...
        <p className="text-sm font-semibold tracking-wide text-slate-500 uppercase">Synchronizing trial balance...</p>
      </div>
    );
  }
    ...
    <div className="p-8 space-y-8 bg-slate-50/50 min-h-screen">
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-slate-500 font-medium">Total real-time ledger audits and payment completion analysis</p>
        </div>
        <button
        ...
        >
          <RefreshCw className="w-3.5 h-3.5" /> Synchronize Ledger
        </button>
      </div>
        ...
        <Card className="border-0 shadow-xl shadow-slate-100/50 rounded-3xl bg-white overflow-hidden">
          <CardHeader className="border-b border-slate-50 px-6 py-5">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <CardTitle className="text-sm font-black uppercase tracking-wider text-slate-500 flex items-center gap-2">
                <Landmark className="w-4 h-4 text-emerald-600" /> Revenue Distribution (GNF)
              </CardTitle>
              {/* Filters */}
