  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setCerts(res.data || []);
      } catch { toast.error('Unable to load certificates'); }
    ...
    <div className="p-[clamp(1rem,2vw+1rem,2rem)] space-y-6 bg-slate-50/50 min-h-screen">
      <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }}>
        <h1 className="text-[clamp(1.2rem,2vw+1rem,1.7rem)] font-black text-slate-900 tracking-tighter flex items-center gap-3 italic">
          MY CHILDREN&apos;S CERTIFICATES <Award className="text-amber-500" size={22}/>
        </h1>
        <p className="text-slate-400 font-bold text-[10px] uppercase tracking-[0.3em] mt-0.5">
          Official diplomas and certificates issued to your children — AMFOFANA High School
        </p>
      </motion.div>
      ...
      <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 }} className="grid grid-cols-2 gap-4">
        {[
          ...
          { label: 'Children', value: Object.keys(byStudent).length, bg: 'bg-amber-50', color: 'text-amber-600', icon: <Users size={16}/> },
        ].map(s => (
      ...
      {loading ? (
        ...
        <Card className="border border-slate-100 shadow bg-white rounded-3xl overflow-hidden">
          <CardContent className="py-20 text-center">
            ...
            <p className="text-[11px] text-slate-300 mt-1">Official diplomas and credentials will appear here once issued to your children</p>
          </CardContent>
        </Card>
      ) : (
                ...
                <Table>
                  <TableHeader className="bg-slate-50">
                    <TableRow className="border-slate-100 hover:bg-transparent">
                      {['Serial No', 'Type', 'Programme / Achievement', 'Honors / GPA', 'Issue Date', 'Status', 'PDF'].map(h => (
