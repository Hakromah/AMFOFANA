...
const LESSON_TYPES: Record<string, { label: string; color: string; bg: string; border: string }> = {
  REGULAR: { label: 'Lecture', color: 'text-blue-700', bg: 'bg-blue-50', border: 'border-blue-200' },
  PRACTICAL: { label: 'TP', color: 'text-emerald-700', bg: 'bg-emerald-50', border: 'border-emerald-200' },
  LAB: { label: 'Lab', color: 'text-teal-700', bg: 'bg-teal-50', border: 'border-teal-200' },
  TUTORIAL: { label: 'TD', color: 'text-indigo-700', bg: 'bg-indigo-50', border: 'border-indigo-200' },
  BREAK: { label: 'Break', color: 'text-amber-700', bg: 'bg-amber-50', border: 'border-amber-200' },
  LUNCH: { label: 'Lunch', color: 'text-orange-700', bg: 'bg-orange-50', border: 'border-orange-200' },
  ASSEMBLY: { label: 'Assembly', color: 'text-purple-700', bg: 'bg-purple-50', border: 'border-purple-200' },
  EXAM: { label: 'Exam', color: 'text-rose-700', bg: 'bg-rose-50', border: 'border-rose-200' },
  STUDY: { label: 'Study', color: 'text-sky-700', bg: 'bg-sky-50', border: 'border-sky-200' },
  SPORTS: { label: 'Sports', color: 'text-lime-700', bg: 'bg-lime-50', border: 'border-lime-200' },
};
  ...
  const handleDownload = () => {
    try {
      const doc = generateTimetable({
        className: className || (student?.username ? `${student.username}` : 'Class Timetable'),
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-[500px]">
      <div className="flex flex-col items-center gap-3">
        ...
        <p className="text-xs font-semibold text-slate-500">Loading timetable...</p>
      </div>
    </div>
  );
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2 text-xs text-slate-400 font-medium mb-2">
            <Link href="/parent/children" className="hover:text-blue-600 transition-colors flex items-center gap-1">
              <ChevronLeft className="w-3.5 h-3.5" /> Children
            </Link>
            <span>/</span>
            ...
            <span>/</span>
            <span className="text-slate-700 font-semibold">Timetable</span>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium">
                {childName ? `${childName} • ` : ''}{className ? `${'Class'}: ${className}` : ''}
              </p>
            </div>
          </div>
        </div>
      ...
      {entries.length === 0 ? (
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-12 text-center flex flex-col items-center justify-center gap-3">
          ...
          <h3 className="font-black text-slate-800 text-lg">No Timetable Published</h3>
          <p className="text-xs text-slate-500 max-w-sm">
            The timetable for this student has not been assigned or published yet by the administration.
          </p>
        </div>
      ) : activeTab === 'daily' ? (
