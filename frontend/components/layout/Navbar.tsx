...
import { getSettings, getNavigation, type NavigationMenuItem } from "@/lib/api";
...
const defaultNavItems = [
  ...
  { name: "Blog", href: "/blog" },
  {
    ...
    href: "/academic",
    subItems: [
      { name: "Academic", href: "/academic", description: "Discover our legacy of excellence since 1990" },
      { name: "login", href: "/login", description: "Meet the team guiding our success" },
    ],
  },
...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    getNavigation().then(nav => {
      ...
      if (items && items.length > 0) {
        setNavItems(items.map((item: NavigationMenuItem) => ({
          name: item.label,
          href: item.url,
          ...(item.children && item.children.length > 0 ? {
            subItems: item.children.map((child: NavigationMenuItem) => ({
              name: child.label,
              href: child.url,
  ...
  const socialLinks = settings.social_links ? JSON.parse(settings.social_links) : defaultSocialLinks;
  const schoolName = settings.school_name || 'A.M. FOFANA';
  ...
  return (
    ...
    >
      <div className="w-full h-full">
        <div className="w-full h-full bg-[#2857AE]">
          <div
            className={`topNav w-full container mx-auto max-w-[1920px] flex items-center  overflow-hidden transition-all duration-500 ease-in-out px-5 md:px-[clamp(20px,5vw,60px)] ${isScrolled ? "h-0 opacity-0" : "h-[37px] opacity-100"
              }`}
          >
            <div className="w-full flex justify-between items-center py-3">
              ...
              <div className="social-media flex items-center space-x-[clamp(10px,2vw,18px)]">
                {socialLinks.map((social: { name: Key | null | undefined; href: string | UrlObject; }) => (
                  <Link
                    key={social.name}
                    href={social.href}
        ...
        <div className="container bg-white mx-auto max-w-[1920px] w-full">
          <div className="w-full bg-white flex h-[calc(var(--header-height)-37px)] items-center px-5 md:px-[clamp(20px,5vw,60px)] relative">
            <div className="w-full h-full flex justify-between items-center">
              ...
              <div
                className={`
                        ...
                        flex flex-col md:flex-row items-center md:gap-[clamp(10px,10vw,70px)] max-md:bg-primary max-md:gap-5
              ...
              >
                      ...
                      <div className="flex items-center md:h-full  justify-between w-full md:w-auto md:justify-start gap-1">
                        {item.subItems ? (
                          <div className="group/sub relative h-full max-md:h-auto max-md:w-full max-md:border-b max-md:border-white/50">
                            ...
                            >
                              <div className="relative max-md:text-white text-[clamp(17px,3vw,18px)] h-full flex items-center md:before:absolute md:before:bottom-0 md:before:left-0 md:before:w-0 md:before:h-0.5 md:before:bg-primary md:before:transition-all md:before:duration-300 md:hover:before:w-full md:hover:before:duration-500 md:hover:text-[#2857AE]">
                                {item.name}
                              </div>
                              <ChevronDown
                          ...
                          >
                            <div className="relative h-full max-md:text-white text-[clamp(17px,3vw,18px)] flex items-center md:before:absolute md:before:bottom-0 max-md:px-5 md:before:left-0 md:before:w-0 md:before:h-0.5 md:before:bg-primary md:before:transition-all md:before:duration-300 md:hover:before:w-full md:hover:before:duration-500 md:hover:text-[#2857AE]">
                              {item.name}
                            </div>
                          </Link>
                        )}
                ...
                <div className="flex flex-col md:flex-row w-full md:w-auto md:gap-[clamp(10px,10vw,40px)] items-center mt-4 md:mt-0 max-md:px-5">
                  <a
                    href={`tel:${schoolPhone.replace(/\s/g, '')}`}
                    className="text-foreground/60 h-auto md:h-full transition-colors lg:hover:text-foreground/80 hidden  md:flex items-center"
                  >
                  ...
                  <div className="w-full md:w-auto shrink-0 max-md:text-primary rounded-full max-md:bg-white">
                    <a
                      href="/contact"
                      className="w-full h-full py-3 px-[clamp(20px,5vw,40px)] flex items-center justify-center transition-colors max-md:text-primary max-md:bg-white lg:hover:bg-primary/10 lg:hover:text-primary bg-primary border border-primary/0 lg:hover:border-primary text-white duration-500 rounded-full text-sm font-medium"
                    >
                      Contact Us
                    </a>
                  </div>
