...
export default function Navbar() {
    ...
    return (
        <header
            className={`sticky top-0 z-50 w-full border-b bg-background/95 transition-transform duration-500 ${isVisible ? 'translate-y-0' : '-translate-y-full'
                }`}
        >
            <div className="w-full h-full container mx-auto max-w-[1920px]">
                ...
                <div className="w-full flex h-[calc(var(--header-height)-37px)] items-center px-5 md:px-[clamp(20px,5vw,60px)] relative">
                    <div className="w-full h-full flex justify-between items-center">
                        <Link href="/" className="mr-6 h-full flex items-center space-x-2 z-50 relative">
                            <div className="flex items-center max-xs:gap-2 gap-4">
                                ...
                                <div>
                                    <h2 className="text-[clamp(18px,2.5vw,24px)] font-semibold text-primary mb-[-3px]">A.M. FOFANA</h2>
                                    <p className="text-[clamp(9px,1.2vw,12px)] text-black tracking-widest uppercase">Islamic & English High School</p>
                                </div>
                            </div>
                        </Link>
                    ...
                    `}>
                            <nav className="flex flex-col md:flex-row md:h-full items-start md:items-center w-full md:w-auto md:gap-5 text-sm font-medium ">
                                {navItems.map((item) => (
                                    <div key={item.name} className="group w-full md:w-auto flex flex-col md:flex-row md:h-full md:items-center relative md:static">
                                        <div className="flex items-center md:h-full  justify-between w-full md:w-auto md:justify-start gap-1">
                                            {item.subItems ? (
                                                <div className="group/sub relative h-full max-md:h-auto max-md:w-full max-md:border-b max-md:border-white/50">
                                                    ...
                                                    <div className={`
                                           gap-from-15 gap-to-20 max-md:grid md:bg-white md:rounded-[15px] md:p-5 scale-100 opacity-100
                                             ...
                                             md:group-hover/sub:delay-200 
                                              relative max-md:w-full
                                               md:absolute md:top-full
                                                md:w-[277px] md:h-fit md:z-[150] md:left-1/2 md:-translate-x-1/2
                                                            ...
                                                            <div className="flex flex-col max-md:w-full h-full md:gap-2 max-md:gap-[1px]">
                                                                {item.subItems.map(sub => (
                                                                    <Link
                                                                        key={sub.name}
                                                                        href={sub.href}
                                                                        className="group/item relative md:pr-[35px] w-full flex justify-between items-center block max-md:bg-white py-2 max-md:px-7 md:px-[18px] md:py-[6px] duration-500 md:hover:text-primary md:rounded-lg md:hover:bg-primary/10 transition-colors max-md:px-5"
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <div className="w-[25px] max-md:hidden h-[25px] absolute right-3 duration-500 scale-75 group-hover/item:scale-100 group-hover/item:opacity-100 opacity-0 top-1/2 -translate-y-1/2 bg-primary rounded-full flex justify-center items-center">
                                                                            <svg className="w-[13px] h-[13px] scale-75 duration-500 group-hover/item:scale-100 group-hover/item:opacity-100 opacity-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 13 13" fill="none">
                                                                                <path d="M12.3136 0.999966C12.3136 0.447681 11.8659 -3.44143e-05 11.3136 -3.39928e-05L2.31363 -3.38664e-05C1.76135 -3.42035e-05 1.31363 0.447681 1.31363 0.999966C1.31363 1.55225 1.76135 1.99997 2.31363 1.99997L10.3136 1.99997L10.3136 9.99997C10.3136 10.5523 10.7613 11 11.3136 11C11.8659 11 12.3136 10.5523 12.3136 9.99997L12.3136 0.999966ZM0.707031 11.6066L1.41414 12.3137L12.0207 1.70707L11.3136 0.999966L10.6065 0.292859L-7.55191e-05 10.8995L0.707031 11.6066Z" fill="white" />
                                                                            </svg></div>
                                                                    </Link>
                                                                ))}
                                                            </div>
                            ...
                            <div className='flex flex-col md:flex-row w-full md:w-auto gap-5 items-center mt-4 md:mt-0 max-md:px-5'>
                                ...
                                <Button className="w-full md:w-auto max-md:text-primary max-md:bg-white">
