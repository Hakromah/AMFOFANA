            ...
            <div className="relative h-[50vh] min-h-[400px]">
                ...
                <div className="absolute inset-0 flex flex-col justify-end container mx-auto px-4 pb-12 text-white">
                    ...
                    <div className="flex items-center gap-4 text-sm font-medium mb-4">
                        <span className="bg-[#2857AE] px-3 py-1 rounded-full">{opportunity.index}</span>
                        <span className="flex items-center gap-1"><Calendar className="w-4 h-4" /> Published: {opportunity.publishedDate}</span>
                        <span className="flex items-center gap-1 text-red-200"><Clock className="w-4 h-4" /> Deadline: {opportunity.deadline}</span>
                    </div>
                            ...
                            <ul className="space-y-3 mb-6">
                                {opportunity.details.requirements.map((req, i) => (
                                    <li key={i} className="flex items-start gap-3">
                                        <CheckCircle className="w-5 h-5 text-[#2857AE] flex-shrink-0 mt-1" />
                                        <span>{req}</span>
                                    </li>
                                ))}
                            </ul>
                        ...
                        <div className="sticky top-24 bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
                            ...
                            <Sheet>
                                <SheetTrigger asChild>
                                    <Button className="w-full bg-[#2857AE] hover:bg-[#1e408a] py-6 text-lg shadow-blue-200 shadow-xl">
                                        Apply Now
                                    </Button>
                                </SheetTrigger>
                                    ...
                                    <div className="px-8 py-10 h-full overflow-y-auto">
                                        <SheetHeader className="mb-8 text-left px-4">
                                            ...
                                            <SheetDescription className="text-base text-gray-500">
                                                Applying for: <span className="font-semibold text-[#2857AE]">{opportunity.title}</span>
                                            </SheetDescription>
                                        </SheetHeader>
                                        ...
                                        <form onSubmit={handleApply} className="space-y-6 px-4">
                                            ...
                                            <Button type="submit" className="w-full bg-[#2857AE] hover:bg-[#1e408a] py-6 text-lg rounded-xl">
